...
def getLemmasAndPosTagsData(sentence_id):
  sql_string = "select start_offset, end_offset, index, word, lemma, postag from sqlnlmatch.nl_features0 "
...
def getDParseData(sentence_id):
  ...
  sql_string += "from sqlnlmatch.nl_features1 as rel, sqlnlmatch.nl_features0 as da1, sqlnlmatch.nl_features0 as da2 "
...
def getWordInfo(sentence_id, index):
  sql_string = "select start_offset, end_offset, word from sqlnlmatch.nl_features0 where sentence_id = " + str(sentence_id) + " and index = " + str(index) + ";"
  curs.execute(sql_string)
  try: return curs.fetchall()[0]
  except: return None

def getNamedEntitiesData(sentence_id):
  sql_string = "select indexes, tag from sqlnlmatch.nl_features2 where sentence_id = " + str(sentence_id) + ";"
  curs.execute(sql_string)
  return curs.fetchall()
...
def getLemmasAndPosTagsData2(sentence_id):
  sql_string = "select start_offset, end_offset, index, word, lemma, postag from sqlnlmatch.sql_features0 "
  sql_string += "where sentence_id = " + str(sentence_id) + " and index > 0 order by index;"
  curs.execute(sql_string)
  return curs.fetchall()
...
def getDParseData2(sentence_id):
  sql_string = "select rel.rel_name, da1.word, da1.start_offset, da1.end_offset, da1.index, da2.word, da2.start_offset, da2.end_offset, da2.index "
  sql_string += "from sqlnlmatch.sql_features1 as rel, sqlnlmatch.sql_features0 as da1, sqlnlmatch.sql_features0 as da2 "
  sql_string += "where rel.sentence_id = da1.sentence_id and rel.left_index = da1.index and "
  sql_string += "rel.sentence_id = da2.sentence_id and rel.right_index = da2.index and "
  sql_string += "rel.sentence_id = " + str(sentence_id) + " "
  sql_string += "order by rel.left_index, rel.right_index"
...
def getWordInfo2(sentence_id, index):
  sql_string = "select start_offset, end_offset, word from sqlnlmatch.sql_features0 where sentence_id = " + str(sentence_id) + " and index = " + str(index) + ";"
  curs.execute(sql_string)
  try: return curs.fetchall()[0]
  except: return None

def getNamedEntitiesData2(sentence_id):
  sql_string = "select indexes, tag from sqlnlmatch.sql_features2 where sentence_id = " + str(sentence_id) + ";"
  curs.execute(sql_string)
  return curs.fetchall()
...
def getSqlNl():
  sql_string = "select id, sentence_id from sqlnlmatch.sqls"
  curs.execute(sql_string)
  return curs.fetchall()
...
#    text = ''
    for (lid, rid) in results:
      insert_query = "insert into sqlnlmatch.alignments (sql_id, sql_idx, sentence_id, sentence_idx) values (" + str(sql_id) + ", " + str(rid) + ", " + str(nl_id) + ", " + str(lid) + ");"
