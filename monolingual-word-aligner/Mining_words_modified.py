...
def align(sid, sentence1, cnlp):
    ...
    try:
      ...
      insert_query = "insert into crowdsourcing.sql_features0 (sentence_id, start_offset, end_offset, index, postag, word, lemma) values (" + str(sid) + ", " + str(start_offset) + ", " + str(end_offset) + ", 0, 'ROOT', 'ROOT', 'root')"
...
#			print (str(start_offset) + '\t\t' +  str(end_offset) + '\t\t' + str(0) + '\t\t' + 'ROOT' + '\t\t' +  'root' + '\t\t' +  'ROOT')
    ...
    for ((start_offset, end_offset), index, word, lemma, postag) in sentence1LemmasAndPosTags:
      insert_query = "insert into crowdsourcing.sql_features0 (sentence_id, start_offset, end_offset, index, postag, word, lemma) values (" + str(sid) + ", " + str(start_offset) + ", " + str(end_offset) + ", " + str(index) + ", '" + postag.replace("'", "''") + "', '" + word.replace("'", "''") + "', '" + lemma.replace("'", "''") + "')"
      curs.execute(insert_query)		
...
#      print (str(start_offset) + '\t\t' +  str(end_offset) + '\t\t' + str(index) + '\t\t' + word + '\t\t' +  lemma + '\t\t' +  postag)
    ...
    for (rel_name, a, b) in dependencyParseAndPutOffsets(sentence1ParseResult):
      ...
      insert_query = "insert into crowdsourcing.sql_features1 (sentence_id, rel_name, left_index, right_index) values (" + str(sid) + ", '" + rel_name.replace("'", "''") + "', " + str(aa[3]) + ", " + str(bb[3]) + ");"
      curs.execute(insert_query)
#      print rel_name + '\t\t' +  aa[0] + '\t\t' + aa[1] + '\t\t' + aa[2] + '\t\t' + aa[3] + '\t\t' +  bb[0] + '\t\t' +  bb[1] + '\t\t' +  bb[2] + '\t\t' +  bb[3]
    ...
    for (offsets, idxs, words, tag) in ner(sentence1ParseResult):
      ...
      for i in range(1, len(idxs)):
        text += ',' + str(idxs[i])
      insert_query = "insert into crowdsourcing.sql_features2 (sentence_id, indexes, tag) values (" + str(sid) + ", '" + text + "', '" + str(tag) + "');" 
      curs.execute(insert_query)
#      text += '\t\t' + str(tag)
#      print (text)
##############################################################################################################################
...
def get_sqls():
  sql_string = "select sq.id, is_valid, sql from crowdsourcing.sqls as sq order by sq.id;"
  curs.execute(sql_string)
  return curs.fetchall()

def get_sentences():
  sql_string = "select id, is_select, sentence from crowdsourcing.sentences order by id;"
  curs.execute(sql_string)
  return curs.fetchall()
